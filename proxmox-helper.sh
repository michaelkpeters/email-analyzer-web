...
#   git clone https://github.com/michaelkpeters/email-analyzer-web.git /opt/sublime-analyzer
...
REPO_URL="${REPO_URL:-https://github.com/michaelkpeters/email-analyzer-web}"
ARCHIVE_URL="${REPO_URL}/archive/refs/heads/main.zip"
# ^^^ Public repo archive URL — no credentials required.
# Or override at runtime: REPO_URL=https://your.repo/url bash proxmox-helper.sh
...
apt-get update -qq
apt-get install -y -qq curl wget rsync unzip
...
# Download latest source from GitHub (public repo, no auth needed)
ARCHIVE="${ARCHIVE_URL}"
TMP_DIR="\$(mktemp -d)"
wget -qO "\$TMP_DIR/repo.zip" "\$ARCHIVE"
unzip -q "\$TMP_DIR/repo.zip" -d "\$TMP_DIR"

mkdir -p /opt
rm -rf /opt/${APP_NAME}
mv "\$TMP_DIR/email-analyzer-web-main" /opt/${APP_NAME}
rm -rf "\$TMP_DIR"
...
direct_install() {
  ...
  msg_info "Installing prerequisites (curl, wget, unzip, python3, rsync)..."
  apt-get install -y -qq curl wget rsync unzip python3 python3-venv

  # Use local files if available, otherwise download from GitHub public archive
  if [ -f "${SCRIPT_DIR}/app/main.py" ]; then
    ...
    msg_info "Downloading from ${ARCHIVE_URL}..."
    TMP_DIR=$(mktemp -d)
    wget -qO "$TMP_DIR/repo.zip" "$ARCHIVE_URL"
    unzip -q "$TMP_DIR/repo.zip" -d "$TMP_DIR"
    rm -rf "$INSTALL_DIR"
    mkdir -p "$INSTALL_DIR"
    mv "$TMP_DIR/email-analyzer-web-main/"* "$INSTALL_DIR/"
    rm -rf "$TMP_DIR"
  fi
